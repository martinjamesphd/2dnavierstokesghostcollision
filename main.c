...
const int grid_max=250000, grid_xmax=500;
...
int main(int argc, char *argv[])
{
	...
	int max_iter, i, j, k, ij;
	int ji, jk, kf, k1, flag, pflag, s_flag;
	double famp, tau_flow, len_flow;
	double energy, epsilon, epsilon_sum;
	FILE *fp, *fp_e;
	...
	fftw_threads_init();			//initializes threads
	omp_set_num_threads(nthreads_omp);
	...
	update_grid(part_main, grid);	//updates position grid
	...
	for(k=0;k<s_flag;k++)
	{
		if(pflag==0)
		{
			...
			sprintf(fname,"field/velProfile%.4d.dat",k);		
			fp=fopen(fname,"w");
		...
		sprintf(fname,"spectra/eSpectra%.4d.dat",k);		
		fp=fopen(fname,"w");
		...
		sprintf(fname,"particle/particle%.4d.dat",k);
		fp=fopen(fname,"w");
		...
		sprintf(fname,"particle/colli_freq%.4d.dat",k);
		fp=fopen(fname,"w");
		for(i=0;i<tau_num;i++)
		{
			for(j=0;j<tau_num;j++)
		...
		{	
			...
			solve_rk2(omega_ft, omega_t, x_vel_ft, y_vel_ft, part_main, colli_freq, grid);    
			find_vel_ft( omega_ft, x_vel_ft, y_vel_ft);
		}
		time=time+dt*t_flag;	
	}
